...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x: int, y: int, hp: int = 100):
        ...
        self.hitbox = pygame.Rect(
            0, 0, self.HITBOX_WIDTH, self.HITBOX_HEIGHT)  # Компактный хитбокс
        self._update_hitbox()
    ...
    def _update_hitbox(self):
        self.hitbox.centerx = self.rect.centerx  # Центрируем хитбокс по центру rect
        ...
        if self.is_attacking:
            if self.attack_timer > 0:
                ...
                if self.frame_idx >= len(self.frames[self.current_attack]):
                    self.is_attacking = False  # Закончили атаку
                    self.state = "IDLE"
                    self.frame_idx = 0
                else:
